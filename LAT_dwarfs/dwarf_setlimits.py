...
def limits(case="J", ndwarf = ["Draco",], deltaEXCL = 3.84, filescan="misc/scan_mV_EV.dat", root = "profiling_case"):
        ...
        if len(ndwarf) == 1:
            ...
            for i, nd in enumerate(ndwarf_mod): # run over dwarf
                ...
                for ns in nscan:
                    ...
                    arrays = np.loadtxt(root + "{}/".format(case) + file)
                    svarr = arrays[:, 0]
                    logL = arrays[:, -1]
...
#            plt.show()
        ...
        print("No case for limits supplied")
        quit()
...
def interactive():
    ...
    limits(case= options.case, ndwarf = listd, deltaEXCL = options.deltaEXCL, filescan = options.filescan)
