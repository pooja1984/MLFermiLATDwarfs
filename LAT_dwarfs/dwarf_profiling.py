...
def DMflux(sv, mDM, Ngammabin, log10Jmean):
    flux = 1/(8.*np.pi)*sv/(mDM**2)*(10**log10Jmean)*Ngammabin
    return flux
...
def DMcount(sv, mDM, Ngammabin, log10Jmean, exp,i,b):
    return float(DMflux(sv, mDM, Ngammabin, log10Jmean[i]))*float(exp[i][b])
...
def scanner(scan_params, dwarf_data, ylist, spectrum, log10J, dJ, lnC, expo, dSph_tag, is_empirical):
    ...
    for sigv in ylist:
        ...
        print('++++ this is dwarf # ',dSph_tag,' sv = ',sigv)
...
def profiling(dwarf_data, case = "J", dwarf_list = ["Draco", ], emp_jlist = "./Jfactors", is_empirical = True, bkg_path = "./bkg_data", svmin=1e-27, svmax = 1e-22, ncpu=4, filescan='misc/scan_mV_EV.dat', spectrum = None):
    ...
    x_voids = np.loadtxt('misc/voids_all_25dSphs.dat')
    count_voids = np.loadtxt('misc/Newbins_counts_voids.dat')
    ...
    svlist=np.logspace(np.log10(svmin),np.log10(svmax),100)
